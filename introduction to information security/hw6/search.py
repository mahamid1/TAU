...
class GadgetSearch(object):
    ...
    def get_format_count(self, gadget_format: str) -> int:
        ...
        return len([i for i in string.Formatter().parse(gadget_format) if(i[1] is not None)])
    ...
    def get_register_combos(self, nregs: int, registers: Tuple[str]) -> Iterable[Iterable[str]]:
        ...
        """
        # TODO: IMPLEMENT THIS FUNCTION
        return [reg for reg in product(list(registers),repeat=nregs)]
    ...
    def format_all_gadgets(self, gadget_format: str, registers: Tuple[str]) -> Iterable[str]:
        ...
        # TODO: IMPLEMENT THIS FUNCTION
        res=[]
        n=self.get_format_count(gadget_format)
        reg_comb=self.get_register_combos(n,registers)
        for comp in (reg_comb):
            res.append(gadget_format.format(*comp))
        return res
    ...
    def find_all(self, gadget: str) -> Iterable[int]:
        ...
        # TODO: IMPLEMENT THIS FUNCTION
        arr=bytearray(self.dump)
        gadget=gadget
        ret_op="RET"
        ret_bytes=assemble.assemble_data(ret_op)
        gadget_bytes=assemble.assemble_data(gadget)+ret_bytes
        size=len(gadget_bytes)
        res=[]
        next_add=self.dump.find(gadget_bytes)
        while next_add!=-1:
            tmp=next_add+int(self.start_addr)
            res.append(tmp)
            next_add=self.dump.find(gadget_bytes,next_add+1)
        return res
    ...
    def find_all_formats(self, gadget_format: str,
        ...
        for gadget in gadgets:
            gadget_app=self.find_all(gadget)
            for address in gadget_app:
                res.append(tuple((gadget,address)))
        return res
